...
(function($){
	...
	function addFile (){
		...
		if($fileArea.data("type")=="docimg"){
			$accept = ".gif,.jpg,.png,.pdf,.doc,.docx,.hwp,.xls,.xlsx,.ppt,.pptx,.zip";
		}else if($fileArea.data("type")=="doc"){
			$accept = ".pdf,.doc,.docx,.hwp,.xls,.xlsx,.ppt,.pptx,.zip";
		}if($fileArea.data("type")=="file"){
			$accept = ".gif,.jpg,.png,.pdf,.doc,.docx,.hwp,.xls,.xlsx,.ppt,.pptx,.zip";
		}
		...
		var html = '<li class="item-file">\
						<div class="input-group">\
							...
							<input type="file" id="'+filegroup+'_' + (cnt-1) + '" class="upload-hidden" accept="'+$accept+'">\
							<input type="text" class="form-control upload-name" placeholder="업로드" readonly="readonly">\
							<span class="input-group-btn input-upload-btn">\
			                     <label for="'+filegroup+'_' + (cnt-1) + '" class="btn btn-dark disabled"><i class="fa fa-upload"></i></label>\
			                 </span>\
						</div>\
				    </li>\
				    ';
    ...
    $(document).on('change','.file_area .upload-hidden', function(){
        ...
        parent.find('.input-upload-btn').children().removeClass('disabled');
    });
})(jQuery);