...
<c:choose>
    <c:when test="${fn:length(list) > 0}">
        ...
        <c:choose>
        <c:when test="${row.progress=='RV02206'}"><%--입주예정--%>
        <c:set var="ctype" value="color1"/>
        </c:when>
        <c:when test="${row.progress=='RV02207'}"><%--입주진행--%>
        <c:set var="ctype" value="color2"/>
        </c:when>
        <c:when test="${row.progress=='RV02208'}"><%--입주완료--%>
        <c:set var="ctype" value="color3"/>
        </c:when>
        </c:choose>
        <li data-pjt_cd="${row.pjt_cd}" data-page-current="${paginationInfo.getCurrentPageNo()}" data-page-last="${paginationInfo.getLastPageNo()}">
            
            <a href="movein_view?pjt_cd=${row.pjt_cd}" class="btn_detail">
                <div class="img_area">
                    <c:choose>
