...
<c:choose>
    <c:when test="${fn:length(list) > 0}">
        ...
        <c:choose>
        <c:when test="${row.progress=='RV02201'}"><%--분양예정--%>
        <c:set var="ctype" value="type1"/>
        </c:when>
        <c:when test="${row.progress=='RV02202'}"><%--분양중--%>
        <c:set var="ctype" value="type2"/>
        </c:when>
        <c:when test="${row.progress=='RV02203'}"><%--분양완료--%>
        ...
        <li data-pjt_cd="${row.pjt_cd}" data-page-current="${paginationInfo.getCurrentPageNo()}" data-page-last="${paginationInfo.getLastPageNo()}">
            ...
            <c:otherwise>
            <a href="javascript:;" class="btn_detail no-data" data-effect="none">
            </c:otherwise>
            </c:choose>
                <div class="img_area">
                    <c:choose>
