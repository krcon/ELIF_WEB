...
<body class="off-canvas-sidebar">
								...
								<div class="row">
									<div class="col-md-10 m-auto">
										<div class="social text-center">
											<h5 class="mt-3 card-description text-center"> 
												<c:choose>
													<c:when test="${type=='initlock'}">
														임시 비밀번호로 로그인 하신 경우 보안을 위해 비밀번호 변경이 반드시 필요합니다.
													</c:when>
													<c:when test="${type=='credential'}">
														비밀번호 사용기간이 90일이 지났습니다. 보안을 위해 비밀번호를 변경해주세요. 
													</c:when>
													<c:otherwise>
										...
										<form name="f" action="/reset/${type}/${token}" method="POST" class="">
    ...
    $(function() {
		$('[name=f]').submit(function(){
			var validate = checkPwd($('[name=password]').val(),$('[name=password_comfirm]').val(),$('[name=old_password]').val(), '');
			return validate;
		});
    });
    $.ajaxSetup({
