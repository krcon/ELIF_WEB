...
<body class="">
						...
						<div class="col-md-12">
							<div class="card">
								<div class="card-header card-header-icon card-header-rose">
									...
									<h4 class="card-title ">${code_type=='krice'?'KR산업 공사단지':'계룡건설 공사단지'} 관리 리스트</h4>
								</div>
								<div class="card-body">
										...
										<table class="table table-striped responsive-utilities jambo_table bulk_action">
											<thead>
												<tr class="headings">
													...
													<th width="10%" class="column-title text-center">상세 페이지</th>
												</tr>
											</thead>
											<tbody>
											<c:choose>
												<c:when test="${fn:length(list) > 0}">
													...
													<tr>
														...
														<td class="text-center">${row.const_status}</td>
														<td class="text-center">${row.progress_name } ${row.progress_const=='Y'?'<span>(미분양 공사중)</span>':'' }</td>
														<td class="text-center">${ct:getMaskedName(row.const_reg_nm)}</td>
														<td class="text-center">
															<button type="button" class="btn-flag-status btn-normal" data-pjt_cd="${row.pjt_cd}">${row.const_view_status }</button>
														</td>
														<td class="text-center">
														<fmt:formatDate var="const_reg_dtm" pattern="yyyy-MM-dd HH:mm" value="${row.const_reg_dtm }"/>
														${const_reg_dtm }
														</td>
														<td class="text-center">
															<c:choose>
															<c:when test="${row.const_status=='Y'&&row.progress!='RV02204'}">
																<a href="#none" data-sub_modpjt_cd="${row.pjt_cd }" data-sub_parentpjt_cd="${row.pjt_cd }">상세정보 관리</a>
															</c:when>
															<c:otherwise>
															</c:otherwise>
															</c:choose>
								...
								<div class="card-footer">
									<div class="w-100 pb-3">
										<div class="row">
											<div class="col-6">
												<a href="management_sort?section=const" class="btn btn-primary">정렬 수정</a>
	...
	(function($){
		...
		$("a[data-sub_modpjt_cd]").on("click", function(e){ //제목 
			e.preventDefault();
			var comSubmit = new ComSubmit();
			comSubmit.setUrl("/mgmt/complex/${code_type}/const_sub_list/"+$(this).data("sub_modpjt_cd"));
		...
		$(document).on('click','.btn-flag-status',function(){
			if(confirm('수정하시겠습니까?')){
				...
				var jsonData = {'pjt_cd':thisEle.data('pjt_cd'),'const_view_status':status}
				$.ajax({
					type: 'POST',
					url: 'const_status',
					data: jsonData,
	...
	function fn_openBoardWrite(obj){
		var comSubmit = new ComSubmit();
		comSubmit.setUrl("/mgmt/complex/${code_type}/const_write");
		comSubmit.submit();
	}
	...
	function fn_openBoardUpdate(obj) {
		var comSubmit = new ComSubmit();
		comSubmit.setUrl("/mgmt/complex/${code_type}/const_modify");
