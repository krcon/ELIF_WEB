...
<body class="">
	<div class="wrapper " data-menu="/mgmt/complex/${code_type}/sales_list">
		<%@ include file="../include/sidebar.jsp"%>
		<div class="main-panel">
			<jsp:include page="../include/top.jsp">
				<jsp:param name="searchHtml" value="complex" />
				<jsp:param name="sub" value="sales" />
			</jsp:include>
			<!-- End Navbar -->
			<div class="content">
						...
						<div class="col-md-12">
							<div class="card">
								<div class="card-header card-header-icon card-header-rose">
									...
									<h4 class="card-title ">${code_type=='krice'?'KR산업 분양단지':'계룡건설 분양단지'} 관리 수정</h4>
								</div>
								<div class="card-body">
									
									<form id="write" action="/mgmt/complex/${code_type}/sales_proc" method="post" encType="multipart/form-data">
										...
										<div class="row">
											<label class="col-sm-2 col-form-label" for="sales_date">분양 일자 <span></span></label>
											<div class="col-sm-10">
												<div class="form-group bmd-form-group">
													<input type="hidden" name="sales_date" class="form-control" value="${map.sales_date}">
													...
													<select name="sales_year" class="selectpicker" data-style="select-with-transition">
													<c:forEach var="i" begin="${syear-10}" end="${syear+10}" step="1" >
													<option value="${i}" ${!empty map.sales_year?(map.sales_year==i?'selected':''):''}>${i}</option>
													</c:forEach>
													</select>
													<select name="sales_month" class="selectpicker" data-style="select-with-transition">
													<c:forEach var="i" begin="1" end="12" step="1" >
													<c:set var="nowmonth"><fmt:formatNumber type="number" minIntegerDigits="2" maxIntegerDigits="2" value="${i}" /></c:set>
													<option value="${nowmonth}" ${!empty map.sales_month?(map.sales_month==nowmonth?'selected':''):(smonth==nowmonth?'selected':'')}>${nowmonth}</option>
													</c:forEach>
													</select>
												</div>
											</div>
											...
											<div class="col-sm-10">
												<div class="form-group bmd-form-group">
													<ul class="file_area mb-0" data-filegroup="pdf">
														<li class="item-file">
															<c:if test="${not empty map.sales_thumb_name}">
															<div class="upload-display"><div class="upload-thumb-wrap"><img src="${map.sales_thumb_path}${map.sales_thumb_name}" class="upload-thumb"></div></div>
															</c:if>
															<span class="bmd-form-group">
																<div class="input-group">
																	<input type="file" name="sales_thumb" id="thumb" class="upload-hidden" accept=".jpg,.jpeg,.gif,.png">
																	<input type="text" name="sales_thumb_old" value="${map.sales_thumb_name}" class="form-control upload-name" placeholder="업로드" readonly="readonly" title="썸네일" data-title="썸네일을">
																	<span class="input-group-btn">
																		<label for="thumb" class="btn btn-primary btn-fab"><i class="fa fa-upload"></i></label>
																		<c:choose>
																		<c:when test="${map.sales_status=='Y'}">
										...
										<div class="row">
											<label class="col-sm-2 col-form-label" for="title_sub">메인 썸네일 <span></span></label>
											<div class="col-sm-10">
												<div class="form-group bmd-form-group">
													<ul class="file_area mb-0" data-filegroup="pdf">
														<li class="item-file">
															<c:if test="${not empty map.sales_main_name}">
															<div class="upload-display"><div class="upload-thumb-wrap"><img src="${map.sales_main_path}${map.sales_main_name}" class="upload-thumb"></div></div>
															</c:if>
															<span class="bmd-form-group">
																<div class="input-group">
																	<input type="file" name="sales_main" id="thumb" class="upload-hidden" accept=".jpg,.jpeg,.gif,.png">
																	<input type="text" name="sales_main_old" value="${map.sales_main_name}" class="form-control upload-name" placeholder="업로드" readonly="readonly" title="썸네일" data-title="썸네일을">
																	<span class="input-group-btn">
																		<label for="thumb" class="btn btn-primary btn-fab"><i class="fa fa-upload"></i></label>
																		<c:choose>
																		<c:when test="${map.sales_status=='Y'}">
										...
										<div class="row">
											<label class="col-sm-2 col-form-label" for="url">분양 사이트 URL <span></span></label>
											<div class="col-sm-10">
												<div class="form-group bmd-form-group">
													<input type="text" name="subhome_url" class="form-control" value="${map.subhome_url}" maxlength="100" placeholder="미입력시 사용안함" title="분양 사이트 URL" data-title="분양 사이트 URL을">
												</div>
											</div>
										</div>
										<div class="row">
											<label class="col-sm-2 col-form-label" for="const_scope">메인 문구 <span></span></label>
											<div class="col-sm-10">
												<div class="form-group bmd-form-group">
													<textarea name="main_comment" class="form-control" style="resize: none; height: 100px;">${map.main_comment}</textarea>
												</div>
												<div class="comment mt-0">* 해당 내용은 메인의 분양 슬라이드에 들어가는 문구입니다.</div>
											</div>
										</div>
										<div class="row">
											...
											<div class="col-sm-10 checkbox-radios">
												<div class="form-check form-check-inline">
													<label class="form-check-label">
														<input class="form-check-input" type="radio" name="sales_view_status" value="Y" ${!empty map.sales_view_status ? ('Y'==map.sales_view_status?'checked':''):'checked' }> 사용
														<span class="circle">
												...
												<div class="form-check form-check-inline">
													<label class="form-check-label">
														<input class="form-check-input" type="radio" name="sales_view_status" value="N" ${'N'==map.sales_view_status?'checked':'' }> 미사용
														<span class="circle">
															<span class="check"></span>
														</span>
													</label>
										...
										<c:if test="${map.sales_reg_nm!=null }">
										<div class="row">
											<label class="col-sm-2 col-form-label" for="START_DTM">작성자 <span></span></label>
											<div class="col-sm-4">
												<div class="form-group bmd-form-group">
													<div class="xdisplay_inputx form-group has-feedback">
														<input type="text" class="form-control" disabled value="${map.sales_reg_nm }" />
													</div>
												</div>
											</div>
											...
											<div class="col-sm-4">
												<div class="form-group bmd-form-group">
													<div class="xdisplay_inputx form-group has-feedback">
														<fmt:formatDate var="crea_dtm" pattern="yyyy-MM-dd HH:mm:ss" value="${map.sales_reg_dtm }"/>
										...
										<c:if test="${map.sales_upda_nm!=null }">
										<div class="row">
											<label class="col-sm-2 col-form-label" for="sales_upda_nm">수정자 <span></span></label>
											<div class="col-sm-4">
												<div class="form-group bmd-form-group">
													<div class="xdisplay_inputx form-group has-feedback">
														<input type="text" class="form-control" disabled value="${map.sales_upda_nm }" />
													</div>
												</div>
											</div>
											...
											<div class="col-sm-4">
												<div class="form-group bmd-form-group">
													<div class="xdisplay_inputx form-group has-feedback">
														<fmt:formatDate var="sales_upda_dtm" pattern="yyyy-MM-dd HH:mm:ss" value="${map.sales_upda_dtm }"/>
														<input type="text" class="form-control" disabled value="${sales_upda_dtm }" />
													</div>
												</div>
											</div>
										</div>
	...
	<form id="commonForm" name="commonForm" method="get"></form>
		...
		function doSubmit(frm) {
			if(confirm('${map.sales_status=='Y'?'수정':'추가'}하시겠습니까?')){
		...
		function fn_openBoardList() {
			if(confirm('목록으로 이동하시겠습니까?')){
				...
				comSubmit.setUrl("/mgmt/complex/${code_type}/sales_list");
		...
		function fn_deleteBoard() {
			...
			var comSubmit = new ComSubmit();
			comSubmit.setUrl("/mgmt/complex/${code_type}/sales_deleteProc");
			if ("${!empty target}" == "true") {
				comSubmit.addParam("target", "${target}");
			}
			if ("${!empty keyword}" == "true") {
