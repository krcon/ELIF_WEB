...
<body class="">
						...
						<div class="col-md-12">
							<div class="card">
								<div class="card-header card-header-icon card-header-rose">
									...
									<h4 class="card-title ">${code_type=='krice'?'KR산업 분양단지':'계룡건설 분양단지'} 관리 리스트</h4>
								</div>
								<div class="card-body">
											...
											<c:choose>
												<c:when test="${fn:length(list) > 0}">
													...
													<tr>
														...
														<td class="text-center">${row.sales_status }</td>
														<td class="text-center">${row.progress_name }</td>
														<td class="text-center">${ct:getMaskedName(row.sales_reg_nm)}</td>
														<td class="text-center">
															<button type="button" class="btn-flag-status btn-normal" data-pjt_cd="${row.pjt_cd}">${row.sales_view_status }</button>
														</td>
														<td class="text-center">
														<fmt:formatDate var="crea_dtm" pattern="yyyy-MM-dd HH:mm" value="${row.sales_reg_dtm }"/>
														${crea_dtm }
														</td>
													</tr>
													</c:forEach>
								...
								<div class="card-footer">
									<div class="w-100 pb-3">
										<div class="row">
											<div class="col-6">
												<a href="management_sort?section=sales" class="btn btn-primary">정렬 수정</a>
											</div>
	...
	(function($){
		...
		$(document).on('click','.btn-flag-status',function(){
			if(confirm('수정하시겠습니까?')){
				...
				var jsonData = {'pjt_cd':thisEle.data('pjt_cd'),'sales_view_status':status}
				$.ajax({
					type: 'POST',
					url: 'sales_status',
					data: jsonData,
	...
	function fn_openBoardWrite(obj){
		...
		comSubmit.setUrl("/mgmt/complex/${code_type}/sales_write");
		comSubmit.submit();
	}
	...
	function fn_openBoardUpdate(obj) {
		var comSubmit = new ComSubmit();
		comSubmit.setUrl("/mgmt/complex/${code_type}/sales_modify");
